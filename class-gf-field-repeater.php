...
/**
** Menashay Givoni (http://customiseit.com.au) changed function get_value_entry_detail. Uploaded on 12-March-2018 
**/
...
class GF_Field_Repeater extends GF_Field
{
    ...
    public static function init_admin()
    {
        ...
        if ($admin_page == 'gf_edit_forms' && !empty($_GET['id'])) {
            add_action('gform_field_standard_settings', array(
                'GF_Field_Repeater',
                'gform_standard_settings'
            ), 10, 2);
            add_action('gform_field_appearance_settings', array(
                'GF_Field_Repeater',
                'gform_appearance_settings'
            ), 10, 2);
            add_action('gform_editor_js_set_default_values', array(
                'GF_Field_Repeater',
                'gform_set_defaults'
            ));
        ...
        if ($admin_page == 'gf_entries') {
            add_filter('gform_form_post_get_meta', array(
                'GF_Field_Repeater',
                'gform_hide_children'
            ));
        }
    ...
    {
        add_action('gform_form_args', array(
            'GF_Field_Repeater',
            'gform_disable_ajax'
        ));
        add_action('gform_enqueue_scripts', array(
            'GF_Field_Repeater',
            'gform_enqueue_scripts'
        ), 10, 2);
        add_filter('gform_pre_render', array(
            'GF_Field_Repeater',
            'gform_unhide_children_validation'
        ));
        add_filter('gform_pre_validation', array(
            'GF_Field_Repeater',
            'gform_bypass_children_validation'
        ));
        add_filter('gform_counter_script', array(
            'GF_Field_Repeater',
            'set_counter_script'
        ), 10, 4);
    }
    ...
    public static function gform_enqueue_scripts($form, $is_ajax)
    {
        if (!empty($form)) {
            if (GF_Field_Repeater::get_field_index($form) !== false) {
                wp_register_script('gforms_repeater_postcapture_js', plugins_url('js/jquery.postcapture.min.js', __FILE__), array(
                    'jquery'
                ), '0.0.1');
                wp_register_script('gforms_repeater_js', plugins_url('js/gf-repeater.js', __FILE__), array(
                    'jquery'
                ), GF_REPEATER_VERSION);
                wp_register_style('gforms_repeater_css', plugins_url('css/gf-repeater.css', __FILE__), array(), GF_REPEATER_VERSION);
                
                wp_localize_script('gforms_repeater_js', 'gfRepeater_php', array(
                    'debug' => GF_REPEATER_DEBUG
                ));
                
                wp_localize_script('gforms_repeater_js', 'GF_Repeater_Phrases', array(
    ...
    {
        return apply_filters('gform_editor_repeater_field_settings', array(
            'admin_label_setting',
            'css_class_setting',
            'description_setting',
                        ...
                        if (in_array($getInputIdNum[1], $repeater_required)) {
                            ...
                            if ($failedValidation) {
                                ...
                                if ($this->errorMessage) {
                                    ...
                                    $this->validation_message = __("A required field was left blank.", 'gravityformsrepeater');
                                }
                                return;
                            }
                        }
                        ...
                        foreach ($inputNames as $inputName) {
                            ...
                            if (!empty($getInputData)) {
                                if (is_array($getInputData)) {
                                    ...
                                    // M.G 24-Apr-2017, bug fix - removed [1] from the brackets
 ...
 public function get_value_entry_detail($value, $currency = '', $use_text = false, $format = 'html', $media = 'screen')
    {
        ...
        * Added format 'pdf', added the html formatting for pdf.
        * Now processes name field and processes address field without the HTML <li> tag.
        * Concatenates these fields just as Gravity Forms does
        */ 
        if (empty($value)) {
            return '';
        } else {
            ...
            foreach ($dataArray as $key => $value) {
                ...
                foreach ($value as $childKey => $childValue) {
                    ...
                    /* M.G. use switch instead of If statement. Makes it easier to read the code */
                    switch ($format) {
                        case 'html':
                    ...
                    if (is_array($childValue)) {
                        if (count($childValue) == 1) {
                            ...
                            if ($form['fields'][$field_index]['type'] == 'date') {
                                ...
                                /* M.G, note the counter 'items'. Used to concatenate name and address fields */
                                $items = 0;                              
                                foreach ($childValue as $childValueData) {
                                    ...
                                    switch ($format) {
                                        case 'html':
                                            ...
                                            break;
                                        case 'pdf':
                                            if ($form['fields'][$field_index]['type'] == 'name') {                              
                                                $childValueOutput .= $childValueData . "&nbsp;";
                                            } else {    
                                                if ($form['fields'][$field_index]['type'] == 'address') {                                                 
                                                  /* city state and post code should be on the same line  */                                                                                              
                                                  if($items == 2 || $items == 3 || $items == 4){
                                                     $childValueOutput .= $childValueData . "&nbsp;";
                                                  } else {
                                                    $childValueOutput .= "<p>" . $childValueData . "</p>";
                                                  }
                                                  $items += 1; 
                                                  if($items >= 6) {$items = 0;};
                                                } else {
    ...
    public function get_value_export($entry, $input_id = '', $format = 'html', $is_csv = false)
    {
        if (empty($input_id)) {
            $input_id = $this->id;
        }
        ...
        switch ($format) {
            case 'text':
                ...
                break;
            case 'html':
                $output = GF_Field_Repeater::get_value_entry_detail($output, '', false, 'html', 'email');
                break;
            case 'pdf':
                $output = GF_Field_Repeater::get_value_entry_detail($output, '', false, 'pdf', 'email');
                break;
            default:
                $output = GF_Field_Repeater::get_value_entry_detail($output, '', false, 'text', 'email');
                $output = preg_replace("/[\r\n]+/", ", ", trim($output));                
